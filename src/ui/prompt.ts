...
export async function select<T>(input: {
  ...
  const fallbackIndex = Math.min(
    Math.max(input.defaultIndex ?? 0, 0),
    options.length - 1,
  );
...
export async function confirm(input: {
  ...
  const answer = (
    await readLine(`${bold(input.message)} ${dim(hint)} `)
  ).toLowerCase();
...
export async function text(input: {
  ...
  const suffix =
    input.defaultValue !== undefined && input.defaultValue.length > 0
